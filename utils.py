...
class NativeScalerWithGradNormCount:
    ...
    def __call__(self, loss, optimizer, clip_grad=None, parameters=None, create_graph=False, update_grad=True, layer_names=None, retain_graph=False):
...
def kmeans_mask(bool_replaced_pos, input_ids, centroids, samples):
    bool_replaced_samples = bool_replaced_pos.repeat_interleave(16, dim=1).repeat_interleave(16, dim=2).expand(3,-1,-1,-1).permute(1,0,2,3)
    samples_replaced = centroids[input_ids].reshape(-1,14,14,3,16,16).permute(0,3,1,4,2,5).reshape(-1,3,224,224)

    return bool_replaced_samples * samples_replaced + (1 - bool_replaced_samples) * samples
