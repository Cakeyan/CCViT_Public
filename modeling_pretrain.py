...
class VisionTransformerForMaskedImageModeling(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, vocab_size=8192, embed_dim=768, depth=12,
        ...
        self.num_patches_one_side = img_size // patch_size
    ...
    def forward(self, x, bool_masked_pos=None, return_all_tokens=False, return_patch_tokens=False, bool_replaced_pos=None):
        ...
        if bool_replaced_pos is not None:
            return self.lm_head(x[bool_replaced_pos])
        else:
...
class VisionTransformerForMaskedImageModelingCLS(VisionTransformerForMaskedImageModeling):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, vocab_size=8192, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=None, init_values=None, attn_head_dim=None,
                 use_abs_pos_emb=True, use_rel_pos_bias=False, use_shared_rel_pos_bias=False, init_std=0.02,
                 early_layers=6, head_layers=2, shared_lm_head=True, use_token_emb=False):
        ...
        if not shared_lm_head:
            ...
            self.cls_pt_lm_head = nn.Linear(embed_dim, patch_size**2 * in_chans)
    ...
    def forward(self, x, bool_masked_pos=None, return_all_tokens=False, return_patch_tokens=False, 
                 return_dual=False, bool_replaced_pos=None):
        ...
        if return_dual:
            return [self.lm_head(x[bool_replaced_pos]), self.lm_head(x_cls_pt) if self.shared_lm_head else self.cls_pt_lm_head(x_cls_pt)]
        if bool_replaced_pos is not None:
            return [self.lm_head(x[bool_replaced_pos]), self.lm_head(x_cls_pt[bool_replaced_pos]) if self.shared_lm_head else self.cls_pt_lm_head(x_cls_pt[bool_replaced_pos])]
        else:
            # return the masked tokens
...
@register_model
def ccvit_base_patch16_224_8k_vocab_cls_pt(pretrained=False, **kwargs):
...
@register_model
def ccvit_base_patch16_224_8k_vocab(pretrained=False, **kwargs):
    if "num_classes" in kwargs:
        _ = kwargs.pop("num_classes")
    if 'vocab_size' in kwargs:
        vocab_size = kwargs['vocab_size']
...
@register_model
def ccvit_large_patch16_224_8k_vocab(pretrained=False, **kwargs):
    if "num_classes" in kwargs:
        _ = kwargs.pop("num_classes")
    if 'vocab_size' in kwargs:
        vocab_size = kwargs['vocab_size']
...
@register_model
def ccvit_large_patch16_224_8k_vocab_cls_pt(pretrained=False, **kwargs):
    if "num_classes" in kwargs:
        _ = kwargs.pop("num_classes")
    if 'vocab_size' in kwargs:
        vocab_size = kwargs['vocab_size']
